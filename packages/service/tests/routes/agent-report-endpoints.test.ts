import { ErrorResponse, ListAgentInstancesResponse, ListHealthChecksResponse } from '@mini-cloud/shared';
import { TaskDispatcher } from '../../src/facades/task-dispatcher';
import { AgentReportEndpoints } from '../../src/routes/agent-report-endpoints';
...
beforeEach(async () => {
  fakes = context();
  server = await TestServer.start(new AgentReportEndpoints({ agentService: fakes.agentService, taskService: fakes.taskService }));
});
...
describe('POST /agent-api/instances', () => {
  it('answers with the instances the service believes an agent still hosts', async () => {
    fakes.taskInstanceDao.seed(anInstance({ instanceId: 'i1', status: 'running' }), anInstance({ instanceId: 'i2', agentId: 'other', status: 'running' }));

    const response = await server.post<ListAgentInstancesResponse>('/agent-api/instances', { agentId: 'mac-mini', status: 'running' });

    // An agent that restarted needs this to resume supervising processes that outlived
    // it — they were spawned detached, so they are still running.
    expect(response.status).toBe(200);
    expect(response.body.instances.map((instance) => instance.instanceId)).toEqual(['i1']);
  });

  it('requires the agent to say who it is', async () => {
    const response = await server.post<ErrorResponse>('/agent-api/instances', { status: 'running' });

    expect(response.status).toBe(400);
    expect(response.body.error).toBe('agentId must be a string');
  });

  it('refuses a status that is not one', async () => {
    // Narrower than the operator's query on purpose: an agent asks about itself, and
    // anything else is a caller that should be using the public listener.
    expect((await server.post('/agent-api/instances', { agentId: 'mac-mini', status: 'nonsense' })).status).toBe(400);
  });
});
