...
export type Gate =
  /** `candidate` seeds the form, so it opens filled in rather than blank. */
  { readonly status: 'setup'; readonly candidate?: Connection } | { readonly status: 'probe'; readonly candidate: Connection };

export function gateFor(candidate: Connection | undefined): Gate {
  if (candidate === undefined) {
    return { status: 'setup' };
  }
  // No token cannot pass, so probing would spend a round trip on a known answer. This
  // is the `?backend=` link's ordinary case: it carries a URL and never a token.
